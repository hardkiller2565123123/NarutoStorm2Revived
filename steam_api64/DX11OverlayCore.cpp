...
void DrawAssetLoadingToast()
{
    ...
    bool taskBusy = AssetTaskQueue::IsBusy();

    if (!scanning && !preloading && !taskBusy)
        return;

    ImGuiViewport* viewport = ImGui::GetMainViewport();

    ImGui::SetNextWindowPos(ImVec2(viewport->Size.x - 430.0f, 36.0f), ImGuiCond_Always);
    ...
    if (taskBusy)
    {
        progress = AssetTaskQueue::GetProgress();
        title = AssetTaskQueue::GetStatus();
    }
    else if (scanning)
    {
        progress = AssetBrowser::GetScanProgress();
        title = "Reading game assets...";
    }
    else
    {
        progress = AssetPreloadManager::GetProgress();
        title = "Loading asset previews...";
    }
    ...
    ImGui::TextWrapped("First load may lag while NS2 Revived caches assets. After this, clicking assets should be smoother.");

    std::string current;

    if (taskBusy)
        current = AssetTaskQueue::GetCurrentTaskName();
    else if (scanning)
        current = AssetBrowser::GetScanStatus();
    else
        current = AssetPreloadManager::GetCurrentAsset();
...
bool CreateDummySwapChain(IDXGISwapChain** outSwapChain)
{
    *outSwapChain = nullptr;
    ...
    DestroyWindow(hwnd);
    UnregisterClassA("NS2RevivedDummyDX11Window", GetModuleHandleA(nullptr));
